...
class SentColBERT(torch.nn.Module):
    def __init__(self, name_or_path, colbert_config=None):
        super().__init__()

        if colbert_config is None:
            self.colbert_config = ColBERTConfig.from_existing(ColBERTConfig.load_from_checkpoint(name_or_path))
        else:
        ...
        self.skiplist_words = model._model_config['skiplist_words']
        self.query_prefix = model._model_config['query_prefix']
        self.document_prefix = model._model_config['document_prefix']
        self.model = model[0].auto_model
        self.dense = model[1].linear
...
class BaseColBERT(torch.nn.Module):
    ...
    def __init__(self, name_or_path, colbert_config=None):
        ...
        if colbert_config is None:
            colbert_config = ColBERTConfig.from_existing(ColBERTConfig.load_from_checkpoint(name_or_path))
        ...
        try:
            ...
            HF_ColBERT = AutoModel.from_pretrained(self.name)
