...
class MultimodalCollectionEncoder():
    def __init__(self, config, checkpoint, is_image=False):
        self.config = config
    ...
    def encode_passages(self, passages):
        ...
        with torch.inference_mode():
            ...
            for passages_batch in batch(passages, self.config.bsize * 50):
                if self.is_image:
                    embs_, doclens_ = self.checkpoint.docFromImage(passages_batch, bsize=self.config.bsize,
...
# class XknowCheckpoint(PreFLMR):
class XknowCheckpoint(MIRe):
    ...
    def query(self, input_ids, attn_mask, images=None, to_cpu=False):
        with torch.no_grad():
            with self.amp_manager.context():
                ...
                Q = super().query(Q_t, Q_i, attn_mask.to(Q_t.device))
